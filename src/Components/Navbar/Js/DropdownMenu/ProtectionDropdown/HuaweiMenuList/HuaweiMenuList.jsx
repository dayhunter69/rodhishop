...
import { options } from "../HuaweiMenuList/setup";
import PhoneCaseSideNav from "../PhoneCase/PhoneCaseSideNav";

const HuaweiMenuList = ({
  phonecaseType,
  handleShowClick,
  handleClick,
  handleClose,
  ...
  const [huaweiType, setHuaweiType] = useState({
    huaweiModel: "",
    huaweiColor: "",
    huaweiPrice: "",
  });
  ...
  const handleSelect = (name, info) => {
    ...
    setHuaweiType((prev) => ({ ...prev, [name]: info.value }));
  };
  const handleClickEnter = () => {
    query.set("huaweiModel", huaweiType.huaweiModel);
    query.set("huaweiColor", huaweiType.huaweiColor);
    query.set("huaweiPrice", huaweiType.huaweiPrice);

    handleClose();
    navigate(`/huawei?${query.toString()}`, { replace: true });
  };
  const isItemSelected = () => {
    return (
      huaweiType.huaweiColor || huaweiType.huaweiModel || huaweiType.huaweiPrice
    );
  };
                ...
                {opt.options.map((info) => (
                  <Typography
                    ...
                    sx={{
                      ...
                      textTransform: "uppercase",
                      color:
                        huaweiType?.[opt.state] !== info.value
                          ? "black"
                          : "rgba(0, 0, 0, 0.49)",
                      cursor: "pointer",
                      border:
